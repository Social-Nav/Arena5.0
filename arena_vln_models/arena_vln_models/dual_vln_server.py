...
from arena_vln_models.backends import DualVLNDecision, DualVLNObservation, Pose2D, create_backend
...
class DualVLNServer(Node):
    """GetCommand server with pluggable dual-vln backends and optional debug visualization."""

    def __init__(self) -> None:
        super().__init__('dual_vln_server')
        ...
        self.declare_parameter('visualization_topic', 'dual_vln/debug_image')
        self.declare_parameter('visualization_rate_hz', 5.0)
        self.declare_parameter('status_topic', 'dual_vln/status')
        ...
        self._last_decision: DualVLNDecision = DualVLNDecision(status='startup', degraded=True)
        ...
        if self._visualization_enabled:
            ...
            if not rgb_topic:
                self.get_logger().warn(
                    'dual_vln visualization enabled but rgb_topic is empty; debug image publishing will stay idle'
                )
        ...
        try:
            self._backend = create_backend(mode=mode, logger=self.get_logger(), params=self._params)
            ...
            self._backend = create_backend(mode='heuristic', logger=self.get_logger(), params=self._params)
        self._fallback_backend = create_backend(mode='heuristic', logger=self.get_logger(), params=self._params)

        self.create_service(GetCommand, 'get_command', self._on_get_command)

        self.get_logger().info(
            (
                'dual_vln_server started '
        ...
        self.get_logger().info(f'dual_vln backend ready: {self._backend.describe()}')
        adapter_available = getattr(self._backend, '_adapter_callable', True) is not None
        self._last_decision = DualVLNDecision(
    ...
    def _build_observation(self) -> DualVLNObservation:
        return DualVLNObservation(
            pose=self._pose,
            goal=self._goal,
    ...
    def _decision_to_twist(self, decision: DualVLNDecision) -> Twist:
        cmd = Twist()
    ...
    def _publish_status(self, decision: DualVLNDecision) -> None:
        message = String()
    ...
    def _set_last_decision(self, decision: DualVLNDecision) -> None:
    ...
    def _start_background_compute(self, observation: DualVLNObservation, now: float) -> bool:
        with self._state_lock:
            if self._compute_in_progress:
                return False
            self._compute_in_progress = True
            # Throttle at dispatch time so a slow CPU InternNav inference does not
            # queue unbounded duplicate requests from Nav2's 20 Hz controller loop.
            self._last_compute_ts = now

        thread = threading.Thread(
            target=self._compute_worker,
            args=(observation,),
            name='dual_vln_compute_worker',
            daemon=True,
        )
        thread.start()
        return True

    def _compute_worker(self, observation: DualVLNObservation) -> None:
        try:
    ...
    def _publish_fallback_while_computing(self, observation: DualVLNObservation) -> None:
        try:
            ...
            self.get_logger().warn(f'fallback command while InternNav inference is running failed: {exc}')

    def _maybe_publish_visualization(self, observation: DualVLNObservation, decision: DualVLNDecision) -> None:
        ...
        try:
            ...
            self.get_logger().warn(f'Failed to publish dual_vln visualization: {exc}')
        ...
        try:
            ...
            if self._should_compute(now):
                if self._backend.uses_model_inference:
                    ...
                    if started:
                        self.get_logger().info(
                            'Started background dual_vln inference; returning cached/fallback command to keep Nav2 responsive.'
                        )
                    with self._state_lock:
